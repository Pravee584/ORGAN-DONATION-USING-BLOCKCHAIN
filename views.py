...
def readDetails(contract_type):
    ...
    deployed_contract_address = '0x6Cb6A5246d408cEeeA6b55eE561D5aFED4B26C14' #hash address to access Organ contract
...
def saveDataBlockChain(currentData, contract_type):
    ...
    deployed_contract_address = '0x6Cb6A5246d408cEeeA6b55eE561D5aFED4B26C14' #contract address
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        contract_abi = contract_json['abi']  # fetch contract's abi - necessary to call its functions
    file.close()
    ...
    if request.method == 'GET':
        ...
        for i in range(len(rows)-1):
            ...
            if user == arr[0]:
                ...
                output+='<td>'+font+str(arr[7])+'</td>'
                output+='<td>'+font+str(arr[8])+'</td>'
                status = re.sub(r'\-\d+' , '',str(arr[9]))
                output+='<td>'+font+status+'</td></tr>'
